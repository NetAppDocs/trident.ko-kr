...
NetApp Trident는 다음을 사용할 때 볼륨 복제를 지원합니다.  `ontap-nas` ,  `ontap-san` ,  `solidfire-san` , 그리고  `gcp-cvs` 스토리지 드라이버. 또는 `ontap-nas-economy` 드라이버를 사용할 때는 `ontap-nas-flexgroup` 복제가 지원되지 않습니다. 기존 볼륨에서 새 볼륨을 생성하면 새 스냅샷이 생성됩니다.
