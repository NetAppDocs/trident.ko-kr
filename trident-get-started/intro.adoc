...
Netapp Trident NetApp 하면 온프레미스 ONTAP 클러스터(AFF, FAS, ASA), ONTAP Select, Cloud Volumes ONTAP, Element 소프트웨어(NetApp HCI, SolidFire), Azure NetApp Files, Amazon FSx for NetApp ONTAP 등 퍼블릭 클라우드나 온프레미스에서 모든 인기 있는 NetApp 스토리지 플랫폼의 스토리지 리소스를 사용하고 관리할 수 있습니다.
