...
* ONTAP-NAS(NFS만 해당), ONTAP-SAN(iSCSI, FCP, NVMe) 및 Google Cloud NetApp Volumes 드라이버에 대한 동시성 지원이 이제 정식 출시(GA)되었습니다.
* Amazon FSx for NetApp ONTAP에 대한 자동 백엔드 구성 지원이 추가되었습니다. 필요한 매개변수를 포함하는 StorageClass를 생성하면 NetApp Trident가 해당 백엔드와 VolumeSnapshotClass(필요한 경우)를 자동으로 생성합니다.
* Azure Government 및 Azure China와 같은 다양한 Microsoft Azure 클라우드에 대한 지원이 추가되었으며, Azure NetApp Files 백엔드에 대한 사용자 지정 클라우드 구성도 추가되었습니다.
* NVMe 볼륨에 대한 컨트롤러 확장성이 향상되었습니다.
* 볼륨 자동 확장 기능에 대한 지원이 추가되었습니다. 사용자 지정 NetApp Trident 자동 확장 정책을 정의하여 자동 확장 동작을 제어할 수 있습니다.
* Google Cloud NetApp Volumes 블록 스토리지(SAN/iSCSI) 지원이 추가되었습니다.
* Google Cloud NetApp Volumes에 대한 자동 계층화 지원이 추가되었습니다. 이제 NetApp Trident를 통해 Google Cloud NetApp Volumes의 ONTAP FabricPool 자동 계층화를 구성하기 위해 Kubernetes-네이티브 제어 기능을 추가하여 스토리지 비용을 최적화할 수 있습니다.
* Kubernetes 1.35에 대한 지원이 추가되었습니다.
