...
** ONTAP-NAS 및 ONTAP-NAS-Economy(두 NAS 드라이버 모두에서 SMB 제외)와 ONTAP-SAN 및 ONTAP-SAN-Economy 드라이버에 대해 강제 볼륨 분리를 통한 자동 워크로드 장애 조치 지원이 추가되었습니다. 보다link:../trident-reco/force-detach.html["강제 분리에 대한 세부 정보"^] .
...
** HTTP 대신 HTTPS로 Prometheus 메트릭을 제공하는 옵션이 추가되었습니다. 보다link:../trident-use/monitor-trident.html["Trident 모니터링"^] .
** 옵션을 추가했습니다 `--no-rename` 볼륨을 가져올 때 원래 이름은 유지하면서 Trident 볼륨의 수명 주기를 관리하도록 합니다. 보다link:../trident-use/vol-import.html["볼륨 가져오기"^] .
