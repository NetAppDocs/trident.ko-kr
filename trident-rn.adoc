...
** Amazon FSx for NetApp ONTAP용 EKS 애드온 및 Helm 기반 설치를 위한 자동 백엔드 구성이 추가되었습니다.
...
* AppMirrorRelationship을 통해 이미 사용 가능한 스토리지 복제에 볼륨:파일 및 볼륨:블록(원시 장치) 스토리지를 위한 KubeVirt/OpenShift 가상화 VM에 대한 백업 및 복구 지원 추가
