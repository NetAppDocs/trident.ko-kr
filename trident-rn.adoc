...
[[whats-new-in-26-06-1]]
== 26.06.1의 새로운 기능

NetApp Trident 및 Trident Protect의 새로운 기능, 개선 사항, 버그 수정 및 사용 중단 예정 기능에 대해 알아보세요.
...
* * Kubernetes *:
+
** CSI 게시와 볼륨 이동 간의 경쟁 조건으로 인해 오래된 액세스 정보를 사용하여 볼륨이 연결될 수 있었던 문제를 수정했습니다.
** NodeGetInfo가 노드 등록이 완료되기 전에 빈 토폴로지 레이블을 반환하여 토폴로지 인식 프로비저닝 실패를 유발하는 경쟁 조건을 수정했습니다.
** NVMe 연결 해제 시 발생하는 드문 경쟁 문제를 수정했습니다. 이 문제로 인해 다른 Pod가 연결 중인 동안 공유 서브시스템이 종료될 수 있었습니다.
** 동시 실행 환경에서 마운트한 볼륨의 클론을 생성할 때 발생하던 NVMe PVC 클론 게시 실패(및 잘못된 상위 네임스페이스 매핑) 문제를 수정했습니다.
** nameTemplate을 사용하는 ONTAP-SAN-Economy 백엔드의 PVC에 대한 스냅샷 생성 실패 문제를 수정했습니다.
...
* * Kubernetes *:
+
** ONTAP 백엔드 구성에 REST 및 ZAPI API 클라이언트 타임아웃을 백엔드별로 설정할 수 있는 선택적 `storageAPITimeout`(초)을 추가했습니다.
...
* * Kubernetes *:
+
** ONTAP-NAS-Economy 및 ONTAP-SAN-Economy 드라이버에 동시성 지원을 일반(GA) 버전으로 추가하여 해당 기능을 기술 미리 보기에서 정식 기능으로 전환했으며, Azure NetApp Files(ANF) 및 SolidFire 드라이버에도 동시성 지원을 일반 버전으로 추가했습니다.
** Kubernetes 1.36에 대한 지원이 추가되었습니다.
** Google Cloud NetApp Volumes(GCNV) NAS 및 SAN 드라이버에서 볼륨 자동 확장에 대한 일반 공급(GA) 지원이 추가되었습니다.
** 새 볼륨을 프로비저닝할 때 최적의 애그리게이트 선택을 위한 Unified ONTAP 균형 배치 기능이 추가되었습니다(link:https://github.com/NetApp/trident/issues/64["문제 #64"]).
** ONTAP-NAS-Flexgroup 드라이버에 ONTAP AFX 지원이 추가되었습니다.
** NetApp Shift Toolkit과의 통합 기능이 추가되어 Red Hat OpenShift Migration Toolkit for Virtualization을 사용할 때 가상 머신 마이그레이션을 간소화합니다.
** GCNV SAN 드라이버에 클라우드 ID 지원이 추가되었습니다.
** Trident Volume Publication(TVP) 전파 및 볼륨 자동 확장 워크플로에 하위 볼륨 지원이 추가되었습니다.
** Trident 동시 실행 모드로 실행할 때 볼륨 업데이트 및 볼륨 그룹 스냅샷 지원이 추가되었습니다.
** GCNV에 동적 gRPC 속도 제한 기능을 추가했습니다.
** 컨트롤러 준비 상태 실행 프로브를 추가하고 컨트롤러 및 사이드카에 대한 기본 Kubernetes API QPS 및 버스트 설정을 조정했습니다.
** 노드 연결, 암호화 설정 및 파일 시스템 포맷 단계를 분리하여 노드 작업 순서를 개선했습니다.
** 보안 강화를 위해 Trident 컨테이너의 루트 파일 시스템을 읽기 전용으로 마운트했습니다.
** ONTAP 백엔드에서 클론 볼륨의 원본 스냅샷이 삭제될 때 클론 볼륨 스냅샷도 최대한 삭제되도록 하는 기능을 추가했습니다.
** Veeam Kasten, Trilio 및 Cohesity에서 생성된 임시 PVC가 ONTAP 볼륨 복구 대기열에 갇히는 것을 방지하기 위해 백업 클론 PVC 인식 기능을 추가했습니다.
** 특수 지역을 지원하기 위해 AWS ARN 처리 기능이 향상되었습니다.
** ONTAP-SAN-ECONOMY 드라이버의 크기 조정 작업 중 Flexvol이 축소되는 것을 방지합니다.
...
* *[Tech Preview]*: ONTAP-NAS-Economy qtree 가져오기 지원이 추가되었습니다.
* *[Tech Preview]*: ASA-R2(SAN 및 NVMe) 드라이버에 동시성 지원이 추가되었습니다.
* *[Tech Preview]*: ONTAP-SAN 및 ONTAP-NAS 드라이버에 GCNV ONTAP 모드 HTTP 전송 기능이 추가되었습니다.
...
* * Kubernetes *:
+
** 게시/게시 취소, 클론, 캐시 및 백엔드 업데이트 작업의 동시 실행에 대한 경쟁 조건 및 상태 처리 문제를 수정했습니다.
** ONTAP-NAS 및 ONTAP-NAS-Economy 드라이버에서 하위 볼륨 및 읽기 전용 클론에 대한 내보내기 정책 경쟁 조건과 동시 게시/게시 취소 처리 문제를 수정했습니다.
** 스냅샷 정책이 다를 때(link:https://github.com/NetApp/trident/issues/1082["문제 #1082"]) MetroCluster 페일오버 후 ONTAP-NAS-Economy FlexVol 검색 문제를 수정했습니다.
** 장시간 실행되는 qtree 삭제를 중단시키는 ONTAP-NAS-Economy 볼륨 삭제 재시도 문제를 수정했습니다(link:https://github.com/NetApp/trident/issues/1121["문제 #1121"]).
** ONTAP-SAN 및 ONTAP-SAN-Economy의 가져오기 및 크기 조정 동작이 수정되었습니다. 여기에는 fsType 유효성 검사, 볼륨 메타데이터 처리 및 자동 증가 모드 동작이 포함됩니다.
** 동시 볼륨 생성 중 NVMe/TCP 네임스페이스 경쟁 조건을 수정했습니다(link:https://github.com/NetApp/trident/issues/1089["문제 #1089"]).
** 사용 가능한 경로와 포털이 일치하지 않을 때 발생하던 iSCSI 크기 조정 실패 문제를 수정하고, 반복적인 크기 조정 동작을 개선했습니다.
** 볼륨 사용 중 상태로 인해 발생하던 ONTAP-SAN 볼륨 삭제 오류를 수정했습니다.
** ONTAP REST 오류 처리 문제를 수정하고 SVM 상태 타임아웃 동작을 개선했습니다.
** GCNV SAN 영역 풀 및 호스트 그룹 처리 문제를 수정하고, 볼륨 식별자 변환을 표준화했으며, 최소 크기 미만의 생성 요청을 표준화했습니다.
** GCNV 네이티브 용량 풀 선택 시 ONTAP 모드 풀을 제외하도록 수정했습니다.
** KubeVirt Trident 자동 워크로드 페일오버를 사용하는 VM에 대한 KubeVirt 페일오버 문제를 수정하여 VM 페일오버 속도를 향상시켰습니다.
** Operator 관리 설치 RBAC 문제(누락된 노드 ClusterRoles 및 업그레이드 중 잘못된 ClusterRole 및 ClusterRoleBinding 조정 오류 포함)를 수정했습니다. 이 수정 사항은 Trident Operator 차트를 사용하는 Operator 및 Helm 기반 설치에 적용됩니다.
** 기본 우선순위 값을 생략하고 YAML 템플릿 들여쓰기를 수정하여 TridentConfigurator 및 관련 CRD에서 지속적으로 발생하던 GitOps diff 문제를 해결했습니다(link:https://github.com/NetApp/trident/issues/915["문제 #915"], link:https://github.com/NetApp/trident/issues/1120["문제 #1120"]).
** Helm 업그레이드 중 TridentVolumePublication CRD의 패치 수정(link:https://github.com/NetApp/trident/issues/1151["문제 #1151"]).
** 노드 등록 재시도 전에 CSI gRPC 서비스를 시작하여 CSI 시작 순서를 수정했습니다.
** 동시 작업 중 보안 SMB 클론 구성 오버레이 문제를 수정했습니다.
** Trident 업그레이드 후 Trident 컨트롤러 포드에서 누락된 주석을 수정했습니다(link:https://github.com/NetApp/trident/issues/1004["문제 #1004"]).
** 대형 SVM에서 느린 qtree 프로비저닝이 개선되었습니다(link:https://github.com/NetApp/trident/issues/1142["문제 #1142"]).
** ControllerPublish가 잘못된 기본 파일 시스템 유형 대신 볼륨 구성 파일 시스템 유형을 사용하도록 수정했습니다.
** 고아 dm 장치로 인해 CSI NodeUnstageVolume 오류가 반복적으로 발생하는 문제를 수정했습니다.
** 소프트 삭제된 노드 정리 후 고스트 다중 경로 아티팩트와 오래된 TridentNode 리소스를 제거하여 오래된 노드 및 다중 경로 정리 기능을 개선했습니다.
...
* OpenShift Service CA Operator 통합 기능을 추가하여 Trident Protect 승인 웹훅에 대한 TLS 인증서 관리를 자동화합니다. Helm 값에서  `useOpenshiftServiceCA`을(를)  `true`로 설정하여 활성화합니다. 자세한 내용은 link:trident-protect/trident-protect-customize-installation.html["Trident Protect 설치 사용자 정의"^]을 참조하십시오.
* KubeVirt VM을 OpenShift 가상화 환경에서 백업 및 복원하는 기능이 추가되었습니다. 네임스페이스 내의 모든 VM을 보호하거나 이름으로 특정 VM을 대상으로 지정하여 백업 또는 스냅샷에서 개별 VM을 선택적으로 복원할 수 있습니다. 자세한 내용은 link:trident-protect/trident-protect-kubevirt-vms.html["KubeVirt 가상 머신을 Trident Protect로 보호합니다"^]을 참조하십시오.
* 리소스 변환 지원이 추가되어 복원 프로세스 중에 Kubernetes 리소스 속성을 추가, 제거 및 수정할 수 있습니다. 자세한 내용은 link:trident-protect/trident-protect-resource-transformations.html["사용자 지정 리소스를 사용하여 리소스를 복원할 때 리소스를 수정합니다"]을 참조하십시오.
*  `storageClassMapping`에 대한 지원이 AppMirrorRelationship(AMR)에 추가되어 단일 AMR 정의 내에서 개별 PVC에 대한 스토리지 클래스 매핑이 가능해졌습니다. 자세한 내용은 link:trident-protect/trident-protect-use-snapmirror-replication.html["NetApp SnapMirror 및 Trident Protect를 사용하여 애플리케이션 복제"^]을 참조하십시오.
...
* 소스 애플리케이션 네임스페이스가 타겟 클러스터에 존재하지 않을 때 승인 웹훅 오류로 인해 클러스터 간 가상 머신 복원이 실패하는 문제를 수정했습니다.
* 기존 애플리케이션의 사용자 지정 리소스와의 사양 불일치로 인해 애플리케이션 생성 단계에서 실패하던 선택적 가상 머신 제자리 복원 문제를 수정했습니다. 이제 기존 애플리케이션이 재사용됩니다.
* 가상 머신과 해당 PersistentVolumeClaim이 모두 삭제된 경우 가상 머신 제자리 복원 작업이 `Running` 상태에서 중단되는 문제를 수정했습니다. 누락된 PersistentVolumeClaim은 이제 치명적인 오류로 처리됩니다.
* 기존 보호 일정을 편집할 때 백업 또는 스냅샷이 즉시 실행되는 오류가 수정되었습니다.
* 스냅샷 최종화 도구가 애플리케이션 사용자 지정 리소스가 삭제된 후에도 해당 리소스를 가져오려고 시도하는 문제를 수정했습니다. 이제 최종화 도구는 애플리케이션을 찾을 수 없는 경우 해당 리소스를 건너뜁니다.
* 스파스 복사를 사용하는 PersistentVolumeClaim 복사 단계에서 가상 머신 인플레이스 복원 후 고정 보호 스토리지 용량이 크게 증가하는 문제가 수정되었습니다.
* ONTAP의 백업 스냅샷 삭제를 확인하기 전에 Kubernetes 스냅샷 객체를 삭제하던 Trident Protect 문제를 수정했습니다. 이제 파이널라이저는  `VolumeSnapshot` 삭제를 기다립니다.
* 가상 머신 동결 작업 실패 시 스냅샷이 진행되지 않던 문제를 수정했습니다. 이제 동결 및 동결 해제 작업이 실패하더라도 계속 진행됩니다.
* AppMirrorRelationship 역방향, 역방향 재동기화 및 재동기화 작업 중에 KubeVirt 가상 머신 애플리케이션이 PersistentVolumeClaims를 잘못 재생성하는 문제를 해결했습니다.




[[whats-new-in-26-02-1]]
== 26.02.1의 새로운 기능
...
* * Kubernetes *:
* 블록(iSCSI) 워크로드에 대해 Flex Unified ZONAL 풀을 지원하도록 Google Cloud NetApp Volumes(GCNV) SAN 동작이 수정되었습니다. 이제 영역 및 지역 구성이 모두 지원됩니다.
* Google Cloud NetApp Volumes(GCNV) NAS UNIFIED 풀 처리 문제를 개선하여 볼륨 생성, 유효성 검사, 이름 지정 및 `InternalID`이 없을 때 대체 볼륨 조회 작업을 향상시켰습니다.
* 사용 가능한 경로와 포털이 일치하지 않을 때 iSCSI 디바이스 크기 조정 작업이 실패할 수 있었던 문제를 수정했습니다.
*  `blkid`가 LUKS 장치의 파일 시스템 유형을 잘못 식별하여 새 볼륨 생성을 방해하던 간헐적인 문제를 수정했습니다.
* 대규모 LUKS 게시 및 스테이징 작업으로 인해 Trident 컨트롤러가 차단되는 문제를 수정했습니다. 이제 사용자는 볼륨과 연결된 LUKS 암호를 추적해야 합니다.
* iSCSI 다중 경로 파티션이 고스트 장치로 나타나 CSI `NodeUnstage` 작업이 성공하지 못하는 문제를 수정했습니다.
* 동시 볼륨 생성 중 NVMe/TCP 네임스페이스 경합 조건을 수정했습니다(Issue #1089).
* 스냅샷이 같은 초 내에 실행될 때 발생했던 동시 클론 스냅샷 작업 문제를 해결했습니다.
*  `dataVolumes`를 사용하는 KubeVirt VM이 Trident 자동 워크로드 페일오버로 페일오버되지 않던 문제를 수정했습니다.
* Trident 동시 실행 코어를 사용하는 동안 유효하지 않은 `TridentMirrorRelationship` 사용자 지정 리소스를 삭제할 때 발생하던 오류를 수정했습니다.
*  `tridentactionmirrorupdates` Kubernetes CRD YAML 정의의 들여쓰기 문제를 수정했습니다(이슈 #1120).
* REST qtree 비동기 삭제 API 호출 문제를 수정했습니다(Issue #1121).
* Fixed `ControllerPublish` 잘못된 기본 파일 시스템 유형 대신 볼륨 구성 파일 시스템 유형을 사용하도록 수정했습니다.
* 컨트롤러 시작 로깅이 동시 드라이버 상태를 올바르게 보고하도록 수정했습니다.
* 동시 실행 Trident에서 일시 중단된 백엔드 처리를 수정했습니다.
* 특수 지역을 지원하기 위해 AWS ARN 처리 기능이 향상되었습니다.
...
보안 취약점을 해결하기 위해 Trident Protect 종속성이 업데이트되었습니다.
...
* Go 런타임: link:https://nvd.nist.gov/vuln/detail/CVE-2025-22871["CVE-2025-22871"^]
* 추가 종속성: link:https://nvd.nist.gov/vuln/detail/CVE-2025-23109["CVE-2025-23109"^]
...
* * Kubernetes *:
+
** ONTAP-NAS(NFS만 해당), ONTAP-SAN(iSCSI, FCP, NVMe) 및 Google Cloud NetApp Volumes(GCNV) 드라이버에 대한 동시성 지원이 정식 출시(GA)되어 이 기능이 Tech Preview에서 졸업했습니다. 자세한 내용은 link:https://docs.netapp.com/us-en/trident/trident-use/controller-scalability.html["컨트롤러 확장성 문서"]을 참조하십시오.
** 사용자 정의 Trident AutoGrow 정책을 기반으로 Trident의 볼륨 자동 확장 지원이 추가되었습니다. 자세한 내용은 link:https://docs.netapp.com/us-en/trident/trident-use/automatic-volume-expansion.html["자동 볼륨 확장 설명서"]를 참조하십시오.
** NVMe 볼륨에 대한 노드 작업의 확장성을 향상시키기 위해 Trident 노드 동시성이 강화되었습니다. 자세한 내용은 link:https://docs.netapp.com/us-en/trident/trident-use/trident-nvme-rwx-subsystem-limits.html["NVMe 볼륨 문서"]를 참조하십시오.
** PVC 주석을 통한 GCNV NAS 볼륨 자동 계층화 지원 추가(`tieringPolicy`, `tieringMinimumCoolingDays`, 풀 선택 및 클론 상속 포함. 자세한 내용은 link:https://docs.netapp.com/us-en/trident/trident-use/gcnv-auto-tiering.html["Google Cloud NetApp Volumes에 대한 자동 계층화 구성 설명서"]을(를) 참조하십시오.
**  `google-cloud-netapp-volumes-san` 드라이버를 사용하여 GCNV SAN 블록(iSCSI) 볼륨에 대한 지원이 추가되었습니다. 여기에는 프로비저닝, LUN 액세스를 위한 노드별 호스트 그룹 매핑 및 볼륨 복제 작업이 포함됩니다. 자세한 내용은 link:https://docs.netapp.com/us-en/trident/trident-use/gcnv-block.html["Google Cloud NetApp Volumes 블록 구성 설명서"]를 참조하십시오.
** Amazon FSx for NetApp ONTAP에 대한 자동 백엔드 구성 지원이 추가되었습니다. 필수 매개변수를 포함하는 StorageClass를 생성하면 Trident는 필요한 경우 해당 백엔드와 VolumeSnapshotClass를 자동으로 생성합니다. 자세한 내용은 link:https://docs.netapp.com/us-en/trident/trident-use/trident-fsx-auto-backend-configuration.html#add-additional-fsxn-file-systems["Amazon FSx for NetApp ONTAP 백엔드 구성 설명서"]를 참조하십시오.
** Azure Government 및 Azure China와 같은 다양한 Microsoft Azure 클라우드에 대한 지원이 추가되었으며, Azure NetApp Files 백엔드에 대한 사용자 지정 클라우드 구성도 추가되었습니다. 자세한 내용은 link:https://docs.netapp.com/us-en/trident/trident-use/anf.html["Azure NetApp Files 백엔드 구성 설명서"]을 참조하십시오.
** Kubernetes 1.35에 대한 지원이 추가되었습니다. 자세한 내용은 link:https://docs.netapp.com/us-en/trident/trident-get-started/requirements.html["요구사항 문서"]을 참조하십시오.
...
* *[Tech Preview]*: ONTAP-NAS-Economy 및 ONTAP-SAN-Economy 드라이버에 대한 동시성 지원이 추가되었습니다.
...
* * Kubernetes *:
+
** ONTAP-NAS, ONTAP-NAS-Economy 및 Google Cloud NetApp Volumes 드라이버에서 읽기 전용 클론을 게시 취소하면 소스 볼륨에서 내보내기 정책 규칙이 제거되는 문제를 수정했습니다link:https://github.com/NetApp/trident/issues/1086["문제 #1086"].
** Bitnami의 공개 이미지 사용 중단에 따른 풀 실패를 방지하기 위해 kubectl 이미지를 경량 Alpine 기반 변형으로 전환했습니다 https://github.com/NetApp/trident/issues/1080["문제 #1080"].
** Trident 업그레이드 중 기존 배포의 주석 보존 문제를 수정했습니다link:https://github.com/NetApp/trident/issues/1004["문제 #1004"].
** 두 스토리지 클래스가 동일한 백엔드를 가리키는 경우 서로 다른 스토리지 클래스 간의 클론 생성을 허용합니다link:https://github.com/NetApp/trident/issues/1104["문제 #1104"].
** 클라우드 환경에서 네트워크 지연으로 인한 시간 초과로 발생한 노드 준비 실패를 수정했습니다. 클라우드 기반 설치에 대한 시간 초과 값을 늘렸습니다.
** LUN 생성 시 프로세스가 재시도 상태에 진입했을 때 파일 시스템 유형 속성이 설정되지 않은 상태로 남아 있던 문제를 수정했습니다.
** REST API 볼륨 조회 시 볼륨 상태를 무시하도록 수정하여 볼륨 쿼리 중 발생하는 오탐을 방지했습니다.
** 대규모 환경에서 ontap-nas-economy 드라이버를 사용할 때 Trident 컨트롤러의 효율성이 향상되었습니다.
** ontap-san-economy 드라이버에서 LUN 가져오기 중에 internalID를 설정합니다.
** 더 많은 서브넷을 처리할 수 있도록 Azure Resource Graph 쿼리 제한이 증가했습니다.
** 일부 백업 애플리케이션에서 경쟁 조건을 방지하기 위해 CSI 및 ONTAP 클론 분할 시간 제한이 개선되었습니다link:https://github.com/NetApp/trident/issues/1098["문제 #1098"], link:https://github.com/NetApp/trident/issues/1100["문제 #1100"].
** LUKS 오류 메시지 억제 문제를 해결했습니다link:https://github.com/NetApp/trident/issues/1069["문제 #1069"].
** iSCSI 및 NVMe 프로토콜 모두에서 오래된 LUKS 매퍼 처리 문제를 수정했습니다. 향상된 정리 로직을 통해 사용되지 않는 장치 매퍼로 인한 마운트 오류를 방지합니다.


* RWX NVMe 볼륨에 대한 확장 제한 사항을 수정했습니다.
* OpenTelemetry-Go 패키지를 업데이트하여 link:https://nvd.nist.gov/vuln/detail/cve-2026-24051["CVE-2026-24051"]를 해결했습니다.
...
* Trident Protect는 이제 제자리 복원 전에 보호 일정을 자동으로 비활성화하고 진행 중인 작업을 취소한 다음 복원이 완료된 후 다시 활성화합니다. 자세한 내용은 link:trident-protect/trident-protect-restore-apps.html["Trident Protect를 사용하여 애플리케이션 복원"^]를 참조하십시오.
* 스케줄 생성 시 즉시 백업 또는 스냅샷을 트리거하기 위해 스케줄 CR에  `runImmediately` 필드를 추가하고  `--run-immediately` CLI 플래그를 추가했습니다. 자세한 내용은 link:trident-protect/trident-protect-protect-apps.html#create-a-data-protection-schedule["데이터 보호 스케줄을 생성합니다"^]을(를) 참조하십시오.
* 복원 CR의 `destinationApplicationName` 필드 또는 `--destination-app-name` CLI 플래그를 사용하여 복원된 애플리케이션에 대한 사용자 지정 이름 지정 지원이 추가되었습니다. 자세한 내용은 link:trident-protect/trident-protect-restore-apps.html["Trident Protect를 사용하여 애플리케이션 복원"^]을(를) 참조하십시오.
...
* 필요한 서비스 계정을 사용할 수 있게 되기 전에 Pod가 생성되어 발생하던 복원 실패 문제를 수정했습니다.
*  `Roles` 및  `RoleBindings`가 애플리케이션 복원 중에 건너뛰어지던 문제가 수정되었습니다.
* 올바르게 구성되었음에도 불구하고  `tridentctl-protect get appvaultcontent` 출력에 원래 클러스터 이름이 표시되지 않던 문제를 수정했습니다.
*  `pipefail` 처리 누락으로 인해 무시되었던 Kopia 복원 오류를 수정했습니다.
* 영구 볼륨을 제외하는 리소스 필터로 인해 발생했던 스냅샷 및 백업 오류를 수정했습니다.
* 네임스페이스 간에 동일한 이름의 PVC가 있는 다중 네임스페이스 애플리케이션에서 잘못된 PVC 복원으로 인해 데이터 손실이 발생할 수 있었던 문제를 수정했습니다.
...
Trident 와 Trident Protect의 새로운 기능, 개선 사항, 수정 사항, 지원 중단 사항을 알아보세요.
...
* * Kubernetes *:
+
** ONTAP -SAN(iSCSI 및 FC) 외에도 ONTAP-NAS NFS 및 ONTAP -SAN-Economy 드라이버에 대한 v1beta1 볼륨 그룹 스냅샷 Kubernetes API를 통해 CSI 볼륨 그룹 스냅샷에 대한 지원이 추가되었습니다. 보다link:https://docs.netapp.com/us-en/trident/trident-use/vol-group-snapshots.html["볼륨 그룹 스냅샷 작업"^] .
** ONTAP-NAS 및 ONTAP-NAS-Economy(두 NAS 드라이버 모두에서 SMB 제외)와 ONTAP-SAN 및 ONTAP-SAN-Economy 드라이버에 대해 강제 볼륨 분리를 통한 자동 워크로드 장애 조치 지원이 추가되었습니다. 보다link:https://docs.netapp.com/us-en/trident/trident-reco/force-detach.html["Trident 사용하여 상태 저장 애플리케이션의 장애 조치 자동화"^] .
** FCP 볼륨의 노드 작업에서 더 높은 확장성을 위해 향상된 Trident 노드 동시성을 제공합니다.
** ONTAP NAS 드라이버에 대한 ONTAP AFX 지원이 추가되었습니다. 보다link:https://docs.netapp.com/us-en/trident/trident-use/ontap-nas-examples.html["ONTAP NAS 구성 옵션 및 예"^] .
** TridentOrchestrator CR 및 Helm 차트 값을 통해 Trident 컨테이너에 대한 CPU 및 메모리 리소스 요청과 제한을 구성하는 지원이 추가되었습니다. (link:https://github.com/NetApp/trident/issues/1000["1000호"] ,link:https://github.com/NetApp/trident/issues/927["927호"] ,link:https://github.com/NetApp/trident/issues/853["853호"] ,link:https://github.com/NetApp/trident/issues/592["592호"] ,link:https://github.com/NetApp/trident/issues/110["110호"] ).
** ASAr2 개성에 대한 FC 지원이 추가되었습니다. 보다link:https://docs.netapp.com/us-en/trident/trident-use/ontap-san-examples.html["ONTAP SAN 구성 옵션 및 예"^] .
** HTTP 대신 HTTPS로 Prometheus 메트릭을 제공하는 옵션이 추가되었습니다. 보다link:https://docs.netapp.com/us-en/trident/trident-use/monitor-trident.html["Trident 모니터링"^] .
** 옵션을 추가했습니다 `--no-rename` 볼륨을 가져올 때 원래 이름은 유지하면서 Trident 볼륨의 수명 주기를 관리하도록 합니다. 보다link:https://docs.netapp.com/us-en/trident/trident-use/vol-import.html["볼륨 가져오기"^] .
** Trident 배포는 이제 시스템 클러스터 중요 우선 순위 클래스에서 실행됩니다.


* Trident 컨트롤러가 helm, operator 및 tridentctl을 통해 호스트 네트워킹을 사용할 수 있는 옵션을 추가했습니다.link:https://github.com/NetApp/trident/issues/858["858호"] ).
...
* *[기술 미리보기]*: ONTAP-SAN 드라이버(통합 ONTAP 9의 iSCSI 및 FCP 프로토콜)에 대한 기존 기술 미리보기 외에도 ONTAP-NAS(NFS 전용) 및 ONTAP-SAN(통합 ONTAP 9의 NVMe)에 대한 동시성 지원이 추가되었습니다.
...
* **쿠버네티스**:
+
** Linux DaemonSet을 node-driver-registrar로 표준화하여 Windows DaemonSet과 컨테이너 이미지 명명 방식을 일치시켜 CSI node-driver-registrar 컨테이너 이름 불일치 문제를 해결했습니다.
** 레거시 Qtree에 대한 내보내기 정책이 제대로 업그레이드되지 않는 문제를 해결했습니다.


* *오픈시프트*:
+
** SCC에서 allowHostDirVolumePlugin이 false로 설정되어 Openshift의 Windows 노드에서 고정 Trident 노드 포드가 시작되지 않는 문제가 해결되었습니다.link:https://github.com/NetApp/trident/issues/950["950호"] ).


* Helm을 통해 설정되지 않는 Kubernetes API QPS가 고정됨(link:https://github.com/NetApp/trident/issues/975["975호"] ).
* 동일한 Kubernetes 노드에서 NVMe 기반 XFS 파일 시스템 PVC의 스냅샷을 기반으로 영구 볼륨 클레임(PVC)을 마운트할 수 없는 문제를 수정했습니다.
* 백엔드별로 고유/공유 서브시스템 이름(예: netappdvp_subsystem)을 추가하여 NDVP 모드에서 호스트/Docker를 재시작한 후 발생하는 UUID 변경 문제를 해결했습니다.
* Trident 23.10 이전 버전에서 24.10 이상 버전으로 업그레이드하는 동안 발생하는 iSCSI 볼륨의 마운트 오류를 ​​수정하여 "잘못된 SANType" 문제를 해결했습니다.
* Trident 컨트롤러를 다시 시작하지 않고도 Trident 백엔드 상태가 온라인/오프라인으로 전환되지 않는 문제가 해결되었습니다.
* PVC 크기 조절 속도가 느려지는 간헐적 경쟁 조건이 수정되었습니다.
* 볼륨 복제 실패 시 고정 스냅샷이 정리되지 않는 문제를 해결했습니다.
* 커널에 의해 장치 경로가 변경되었을 때 볼륨의 스테이지 해제에 실패하는 문제가 수정되었습니다.
* LUKS 장치가 이미 닫혀서 볼륨을 스테이지 해제하지 못하는 문제가 수정되었습니다.
* 느린 저장 작업으로 인해 ContextDeadline 오류가 발생하는 문제가 해결되었습니다.
* Trident Operator는 구성 가능한 k8s-timeout을 기다려 Trident 버전을 확인합니다.
...
* 일정 및 백업 CR에 대한 스냅샷 CR 시간 초과를 제어하기 위한 주석이 추가되었습니다.
+
...
+
보다link:https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-protect-apps.html["지원되는 백업 및 일정 주석"^] .



* 백업 CR에서 사용될 PVC 바인딩 시간 초과를 구성하기 위해 일정 CR에 주석을 추가했습니다. `protect.trident.netapp.io/pvc-bind-timeout-sec` . 보다link:https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-protect-apps.html["지원되는 백업 및 일정 주석"^] .
* 개선됨 `tridentctl-protect` 실행 후크 실패를 나타내는 새 필드가 추가되어 백업 및 스냅샷 목록이 추가되었습니다.
...
** iSCSI 외에도 NVMe/TCP용 ONTAP ASA r2에 대한 지원이 추가되었습니다. 보다link:https://docs.netapp.com/us-en/trident/trident-use/ontap-san-examples.html["ONTAP SAN 구성 옵션 및 예"^] .
...
**  `nfsMountOptions`가 설정되지 않은 경우 GCNV 볼륨이 NFSv3로 기본 설정되던 문제를 수정했습니다. 이제 NFSv3 및 NFSv4 프로토콜을 모두 지원합니다.  `nfsMountOptions`가 제공되지 않으면 호스트의 기본 NFS 버전(NFSv3 또는 NFSv4)이 사용됩니다.
