...
NOTE: 설치 프로그램이 성공적으로 완료되지 않거나 `trident-controller-<generated id>` (`trident-csi-<generated id>`에 *실행 중* 상태가 표시되지 않으면 플랫폼이 설치되지 않은 것입니다.  `-d`를 사용하여 link:../troubleshooting.html#unsuccessful-trident-deployment-using-tridentctl["디버그 모드를 켭니다"]하고 문제를 해결하세요.
...
[%collapsible]
====
처리량 향상을 위해 Trident 컨트롤러의 동시 작업을 활성화하려면 이 예시와 같이 설치 중에  `--enable-concurrency` 옵션을 추가하십시오. 지원되는 드라이버에 대한 자세한 내용은 링크: https://docs.netapp.com/us-en/trident/trident-use/controller-scalability.html#controller-scalability[Controller Scalability documentation]를 참조하십시오.
