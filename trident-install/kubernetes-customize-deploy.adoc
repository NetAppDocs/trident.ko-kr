...
| 트리덴티이미지 | 설치할 Trident 이미지 | `"netapp/trident:26.06"` 
...
| Prometheus 메트릭 엔드포인트에 HTTPS를 활성화합니다. | 거짓 | 호스트 네트워크 
|===
...
WARNING: *면책 조항*: 이 예제에서 제공된 요청 및 제한 값은 데모 목적으로만 사용됩니다. 환경과 작업 부하 요구 사항에 따라 이러한 값을 조정하세요.
