...
----
.  `helm install`를 사용하여 배포 및 이미지 레지스트리 위치에 이름을 지정하십시오. link:../trident-get-started/requirements.html#container-images-and-corresponding-kubernetes-versions["Trident 및 CSI 이미지"]는 하나의 레지스트리 또는 여러 레지스트리에 위치할 수 있지만, 모든 CSI 이미지는 동일한 레지스트리에 있어야 합니다. 예시에서  `100.2602.0`는 설치하려는 Trident 버전입니다.
+
[role="tabbed-block"]
====
.하나의 레지스트리에 있는 이미지
