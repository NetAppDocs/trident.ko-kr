...
[]
=====
* 이제 Kubernetes가 Trident에서 지원됩니다. Kubernetes를 업그레이드하기 전에 Trident를 먼저 업그레이드하십시오.
...
본인에게 맞는 설치 방법을 선택하세요. 결정하기 전에 link:kubernetes-deploy.html#moving-between-installation-methods["방법 간 이동"]에 대한 고려 사항도 검토해야 합니다.
...
수동으로 배포하든 Helm을 사용하든, Trident operator는 설치를 간소화하고 Trident 리소스를 동적으로 관리하는 훌륭한 방법입니다. 속성을 사용하여 link:kubernetes-customize-deploy.html["Trident 운영자 배포를 사용자 지정합니다"]도 할 수 있으며, 이는  `TridentOrchestrator` 커스텀 리소스(CR)에서 가능합니다.
