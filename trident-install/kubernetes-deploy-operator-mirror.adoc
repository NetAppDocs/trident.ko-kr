...
=== 3단계: 운영자의 레지스트리 위치를 업데이트합니다
 ...
 `/deploy/operator.yaml`에서  `image: docker.io/netapp/trident-operator:26.06.0`를 업데이트하여 이미지 레지스트리의 위치를 반영하십시오. link:../trident-get-started/requirements.html#container-images-and-corresponding-kubernetes-versions["Trident 및 CSI 이미지"]는 하나의 레지스트리 또는 여러 레지스트리에 위치할 수 있지만, 모든 CSI 이미지는 동일한 레지스트리에 있어야 합니다. 예를 들면 다음과 같습니다.

* `image: <your-registry>/trident-operator:26.06.0` 이미지가 모두 하나의 레지스트리에 있는 경우.
* `image: <your-registry>/netapp/trident-operator:26.06.0` Trident 이미지가 CSI 이미지와 다른 레지스트리에 있는 경우.
