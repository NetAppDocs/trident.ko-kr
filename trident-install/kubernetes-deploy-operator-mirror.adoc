...
~ 안에 `/deploy/operator.yaml` , 업데이트 `image: docker.io/netapp/trident-operator:25.10.0` 이미지 레지스트리의 위치를 반영합니다. 당신의link:../trident-get-started/requirements.html#container-images-and-corresponding-kubernetes-versions["Trident 및 CSI 이미지"] 하나의 레지스트리나 여러 레지스트리에 위치할 수 있지만, 모든 CSI 이미지는 동일한 레지스트리에 위치해야 합니다. 예를 들어:

* `image: <your-registry>/trident-operator:25.10.0`모든 이미지가 하나의 레지스트리에 있는 경우.
