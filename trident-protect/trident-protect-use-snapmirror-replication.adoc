...
--
====
. 선택적으로 소스 클러스터에서 소스 애플리케이션의 스냅샷을 만듭니다. 이 스냅샷은 대상 클러스터의 애플리케이션의 기반으로 사용됩니다. 이 단계를 건너뛰면 다음 예약된 스냅샷이 실행될 때까지 기다려야 최신 스냅샷을 얻을 수 있습니다. 주문형 스냅샷을 생성하려면 다음을 참조하세요. link:https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-protect-apps.html#create-an-on-demand-snapshot["필요 시 스냅샷을 생성합니다"^].
...
NOTE: CR을 사용할 때는 CR을 적용하기 전에 대상 네임스페이스를 수동으로 생성해야 합니다. Trident Protect는 CLI를 사용할 때만 네임스페이스를 자동으로 생성합니다.
...
*** *spec.storageClassMapping*: (_선택 사항_) 동일한 AMR 내의 개별 PVC에 대해 소스 스토리지 클래스를 타겟 스토리지 클래스에 매핑합니다. 각 항목에는  `source` 및  `destination` 스토리지 클래스 이름이 필요합니다. 설정된 경우,  `storageClassMapping`은(는)  `storageClassName`을(를) 완전히 재정의하며, 일치하는 소스 항목이 없는 PVC는  `storageClassName`이(가) 아닌 클러스터 기본 스토리지 클래스를 사용합니다.
+
 `storageClassMapping`을 AppMirrorRelationship CR에서 사용할 때는 다음과 같은 제한 사항에 유의하십시오.

+
**** `storageClassMapping`이(가)  `storageClassName`을(를) 완전히 덮어씁니다. 두 필드가 모두 설정된 경우  `storageClassName`은(는) 무시됩니다.
**** 에 일치하는 항목이 없는 소스 스토리지 클래스를 가진 PVC는 `storageClassMapping` 클러스터 기본 스토리지 클래스를 사용하며, `storageClassName`은(는) 사용하지 않습니다.
**** 각 항목에는  `source`와  `destination` 값이 모두 필요합니다.
**** 대상 스토리지 클래스를 변경하면 기존 대상 PVC가 삭제되고 다시 생성될 수 있습니다.
...
NOTE:  `--storageclass-mapping`을 사용하여 개별 소스 스토리지 클래스를 대상 스토리지 클래스에 매핑합니다. 쉼표로 구분하여 하나 이상의  `source:destination` 쌍을 지정합니다(예:  `"class-a:class-b,class-c:class-d"`).  `*:destination-class`를 와일드카드로 사용하여 매핑되지 않은 모든 PVC를 단일 대상 스토리지 클래스에 매핑합니다.  `--storageclass-mapping`가 설정된 경우  `--storage-class`는 무시됩니다.

+
*단일 대상 스토리지 클래스를 사용하는 예시 (`--storage-class`):*
