...
애플리케이션에서 데이터 보호 작업을 수행할 때 사용할 AppVault CR을 만들어야 하며, AppVault CR은 Trident Protect가 설치된 클러스터에 있어야 합니다. AppVault CR은 사용자 환경에 따라 다릅니다. AppVault CR의 예는 다음을 참조하세요.link:trident-protect-appvault-custom-resources.html["AppVault 사용자 지정 리소스."]
...
NOTE: 전체 네임스페이스가 아닌 이름으로 특정 KubeVirt 가상 머신(VM)을 보호하는 VM 범위 애플리케이션을 정의하려면 애플리케이션 CR의 `spec.includedVirtualMachines` 필드 또는 CLI의 `--virtual-machines` 플래그를 사용하십시오. 자세한 내용, 필드 참조 및 YAML 예제는 link:trident-protect-kubevirt-vms.html["KubeVirt 가상 머신을 Trident Protect로 보호합니다"]을 참조하십시오.
...
. (_선택 사항_) 필요한 경우 동일한 CR에 리소스 필터링을 추가하여 특정 리소스를 포함하거나 제외할 수 있습니다.
+
** *일반 필터 예:*
+
...
** *PVC 전용 필터 예:*
+
PVC 전용 애플리케이션을 정의하려면 리소스 필터에  `PersistentVolume` 및  `VolumeSnapshotClass`도 포함해야 합니다. 스냅샷 및 백업 작업은  `PersistentVolume`(각 PVC에 바인딩된 클러스터 범위 볼륨) 및  `VolumeSnapshotClass`(스냅샷 드라이버)에 따라 달라지며 이들이 없으면 실패합니다. 예를 들면 다음과 같습니다.
...
+ * 애플리케이션을 생성하고 파일 시스템 고정 동작에 대한 전역 설정을 사용하십시오.
...
----
+ * 애플리케이션을 생성하고 파일 시스템 동결 동작에 대한 로컬 애플리케이션 설정을 구성합니다.
...
----
*  `--resource-filter-include` 및  `--resource-filter-exclude` 플래그를 사용하여 다음 예시와 같이 그룹, 종류, 버전, 레이블, 이름, 네임스페이스 등의  `resourceSelectionCriteria`을(를) 기준으로 리소스를 포함하거나 제외할 수 있습니다.
...
----
+ * PVC 전용 애플리케이션을 정의하려면  `PersistentVolume` 및  `VolumeSnapshotClass``도 리소스 필터에 포함해야 합니다. 스냅샷 및 백업 작업은  `PersistentVolume`(각 PVC에 바인딩된 클러스터 범위 볼륨) 및  `VolumeSnapshotClass`(스냅샷 드라이버)에 따라 달라지며, 이들이 없으면 실패합니다. 예를 들면 다음과 같습니다.
