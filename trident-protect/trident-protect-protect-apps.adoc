...
** *spec.runImmediately*: (_선택 사항_)  `true`로 설정하면 스케줄 생성 시 일회성으로 즉시 기준선 실행(보존 설정에 따른 백업 및/또는 스냅샷)을 트리거합니다. 기본값은  `false`입니다. 이 설정은 이후 반복 실행에는 영향을 미치지 않습니다.
+
...
+
** *전체 백업 예약*: 다음을 사용하세요 `--full-backup-rule` 증분 백업이 아닌 전체 백업을 예약하려면 플래그를 지정하세요. 이 플래그는 다음 경우에만 작동합니다. `--granularity Daily`. 가능한 값:
+
*** `Always`매일 전체 백업을 생성하세요.
*** 특정 요일 지정: 쉼표로 구분하여 하나 이상의 요일을 지정하세요(예: `"Monday,Thursday"`). 유효한 값: 월요일, 화요일, 수요일, 목요일, 금요일, 토요일, 일요일.
+

NOTE: 그만큼 `--full-backup-rule` 해당 플래그는 시간별, 주별 또는 월별 세분화에서는 작동하지 않습니다.



** *즉시 기준선 보호*:  `--run-immediately true`을 사용하여 첫 번째 예약 실행 시간을 기다리지 않고 예약이 생성될 때 즉시 초기 백업 또는 스냅샷을 생성합니다. 기본값은  `false`입니다.
** *스냅샷 전용 일정*: 설정 `--backup-retention 0` 그리고 0보다 큰 값을 지정하십시오. `--snapshot-retention`.
...
| protect.trident.netapp.io/전체 백업 규칙 | 문자열 | 전체 백업 예약 규칙을 지정합니다. 다음과 같이 설정할 수 있습니다. `Always` 지속적인 전체 백업을 원하시거나 필요에 따라 맞춤 설정할 수 있습니다. 예를 들어, 일별 세분성을 선택하면 전체 백업이 수행될 요일을 지정할 수 있습니다(예: `"Monday,Thursday"`). 유효한 요일 값은 월요일, 화요일, 수요일, 목요일, 금요일, 토요일, 일요일입니다. 참고로 이 주석은 특정 조건을 충족하는 일정에만 사용할 수 있습니다. `granularity` 설정됨 `Daily`. | 설정되지 않음(모든 백업은 증분 백업임) 
