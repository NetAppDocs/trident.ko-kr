...
AutoSupport 설정, 네임스페이스 필터링 및 OpenShift 웹훅 인증서 관리를 특정 요구 사항에 맞게 사용자 지정할 수 있습니다. 다음 표는 사용 가능한 구성 매개변수를 설명합니다.
...
| useOpenshiftServiceCA | 부울 | OpenShift의 기본 서비스 CA Operator와의 통합을 활성화하여 Trident Protect 승인 웹훅에 대한 TLS 인증서를 자동으로 생성하고 주입합니다. 이 옵션을 `true`로 설정하면 서비스 CA Operator가 웹훅 TLS 인증서를 자동으로 관리합니다. OpenShift 클러스터에서만 사용하십시오. (기본값: `false`) 
|===
