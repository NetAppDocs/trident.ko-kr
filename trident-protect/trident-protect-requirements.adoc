...
=== KubeVirt 사용 시 고려 사항
 ...
 https://kubevirt.io/["KubeVirt"^]SnapMirror 복제와 함께 가상 머신을 사용하려면 SVM을 고정 및 고정 해제할 수 있도록 가상화를 설정해야 합니다. 가상화를 설정하면 SVM에서 사용 중지 및 사용 중지를 푸는 데 필요한 툴이 포함됩니다. 가상화 설정에 대한 자세한 내용은 을 https://docs.openshift.com/container-platform/4.17/virt/install/installing-virt.html["OpenShift Virtualization 설치"^]참조하십시오.
