...
[role="lead"]
Trident 설치와 함께 ONTAP SAN 드라이버를 생성하고 사용합니다. 이 섹션에서는 백엔드를 StorageClasses에 매핑하기 위한 백엔드 구성 예제와 세부 정보를 제공합니다. link:https://docs.netapp.com/us-en/asa-r2/get-started/learn-about.html["ASA r2 시스템"^]은(는) 스토리지 계층 구현 방식에서 다른 ONTAP 시스템(ASA, AFF, FAS)과 다릅니다. 이러한 차이점은 아래 표에 표시된 특정 매개변수 사용에 영향을 미칩니다. link:https://docs.netapp.com/us-en/asa-r2/learn-more/hardware-comparison.html["ASA r2 시스템과 다른 ONTAP 시스템 간의 차이점에 대해 자세히 알아보세요."^]. Trident 백엔드 구성에서 시스템이 ASA r2임을 명시적으로 지정할 필요는 없습니다.  `ontap-san`을(를)  `storageDriverName`로 선택하면 Trident가 ASA r2 또는 다른 ONTAP 시스템을 자동으로 감지합니다. 아래 표에 표시된 것처럼 일부 백엔드 구성 매개변수는 ASA r2 시스템에는 적용되지 않습니다.
...
26.06 릴리스부터 ONTAP 균형 배치 기능을 활성화하여 ONTAP이 새 볼륨에 대한 애그리게이트를 선택하도록 할 수 있습니다. 요구 사항 및 예제는 link:ontap-balanced-placement.html["ONTAP 백엔드와 함께 균형 잡힌 배치 사용"]을 참조하십시오.
...
| 'torageDriverName'입니다 | 스토리지 드라이버의 이름입니다. 생성 후에는 수정할 수 없습니다. 이 매개변수를 업데이트하려면 새 백엔드를 생성해야 합니다. | `ontap-san` 또는 `ontap-san-economy` 
...
| '토르agePrefix' | SVM에서 새 볼륨을 프로비저닝할 때 사용되는 접두사입니다. 나중에 수정할 수 없습니다. 이 매개변수를 업데이트하려면 새 백엔드를 생성해야 합니다. | `trident` 
...
NOTE: SVM에서 애그리게이트가 업데이트되면 Trident 컨트롤러를 재시작하지 않고도 SVM을 폴링하여 Trident에 자동으로 업데이트됩니다. 볼륨을 프로비저닝하기 위해 Trident에서 특정 애그리게이트를 구성한 경우, 해당 애그리게이트의 이름이 변경되거나 SVM에서 이동되면 SVM 애그리게이트를 폴링하는 동안 백엔드가 Trident에서 실패 상태로 전환됩니다. 백엔드를 다시 온라인 상태로 전환하려면 SVM에 있는 애그리게이트로 변경하거나 해당 애그리게이트를 완전히 제거해야 합니다.
...
| `formatOptions`  a| 
 `formatOptions`을 사용하여 `mkfs` 명령에 대한 명령줄 인수를 지정합니다. 이 인수는 볼륨을 포맷할 때마다 적용됩니다. 이를 통해 원하는 대로 볼륨을 포맷할 수 있습니다. 장치 경로를 제외하고 mkfs 명령 옵션과 유사하게 formatOptions를 지정해야 합니다. 예: "-E nodiscard"
...
* *-E nodiscard(ext3, ext4):* mkfs 시점에 블록을 삭제하려고 시도하지 마세요(처음에 블록을 삭제하는 것은 솔리드 스테이트 장치와 스파스/씬 프로비저닝 스토리지에서 유용합니다). 이는 더 이상 사용되지 않는 옵션인 "-K"를 대체하며 ext3, ext4 파일 시스템에 적용할 수 있습니다.
...
| 암호화 | NetApp 볼륨 암호화(NVE)를 새 볼륨에서 활성화합니다. 기본값은 `false`입니다. 이 옵션을 사용하려면 클러스터에서 NVE 라이선스가 있고 활성화되어 있어야 합니다. 백엔드에서 NAE가 활성화된 경우 Trident에서 프로비저닝된 모든 볼륨은 NAE가 활성화됩니다. 자세한 내용은 다음을 참조하십시오. link:../trident-reco/security-reco.html["Trident가 NVE 및 NAE와 작동하는 방법"]. | "false" *지정된 경우 설정됨  `true` ASA r2 시스템*용. 
...
다음 StorageClass 정의는 <<가상 풀의 백엔드 예>>을 참조합니다.  `parameters.selector` 필드를 사용하여 각 StorageClass는 볼륨을 호스팅하는 데 사용할 수 있는 가상 풀을 지정합니다. 이제 볼륨은 선택된 가상 풀에 정의된 속성을 갖게 됩니다.

* The `protection-gold` StorageClass는 `ontap-san` 백엔드의 첫 번째 가상 풀에 매핑됩니다. 이 풀만이 골드 레벨 보호를 제공합니다.
+
[source, yaml]
----
...
parameters:
  ...
  fsType: "ext4"
----
* The `protection-not-gold` StorageClass는 `ontap-san` 백엔드의 두 번째 및 세 번째 가상 풀에 매핑됩니다. 이 풀들만이 골드 수준 외에 다른 보호 수준을 제공합니다.
+
[source, yaml]
----
apiVersion: storage.k8s.io/v1
...
parameters:
  ...
  fsType: "ext4"
----
* The `app-mysqldb` StorageClass는 `ontap-san-economy` 백엔드의 세 번째 가상 풀에 매핑됩니다. 이 풀은 mysqldb 유형 애플리케이션에 대한 스토리지 풀 구성을 제공하는 유일한 풀입니다.
+
[source, yaml]
----
apiVersion: storage.k8s.io/v1
...
parameters:
  ...
  fsType: "ext4"
----
* The `protection-silver-creditpoints-20k` StorageClass는 `ontap-san` 백엔드의 두 번째 가상 풀에 매핑됩니다. 이 풀은 실버 등급 보호와 20000 크레딧 포인트를 제공하는 유일한 풀입니다.
+
[source, yaml]
----
apiVersion: storage.k8s.io/v1
...
parameters:
  ...
  fsType: "ext4"
----
* The `creditpoints-5k` StorageClass는 `ontap-san` 백엔드의 세 번째 가상 풀과 `ontap-san-economy` 백엔드의 네 번째 가상 풀에 매핑됩니다. 5000 크레딧 포인트를 제공하는 풀은 이 두 가지뿐입니다.
+
[source, yaml]
----
apiVersion: storage.k8s.io/v1
...
parameters:
  ...
  fsType: "ext4"
----
* The `my-test-app-sc` StorageClass는 `testAPP` `ontap-san`드라이버의 가상 풀에 `sanType: nvme`로 매핑됩니다. 이것이 유일하게 `testApp`를 제공하는 풀입니다.
+
[source, yaml]
----
---
