...
25.02 릴리즈부터 Trident는 ONTAP 9.16.1 이상의 ASA R2 속성을 자동으로 감지하고 iSCSI 프로토콜을 통해 ASA R2의 스토리지 용량 할당 및 사용을 지원합니다.
...
NOTE: Trident에서는 하나 이상의 애그리게이트가 SVM에 할당되어야 합니다. ASA R2를 사용하는 경우 이 작업은 모드에서 ONTAP CLI를 통해서만 사용할 수 `diag` 있습니다.
