...
StorageClass를 통해 선택적 백엔드 매개 변수를 전달할 수 있습니다. StorageClass `parameters` 섹션에서 모든 선택적 값을 문자열로 정의하십시오. 백엔드 매개변수의 전체 목록은 다음을 참조하십시오: link:trident-fsx-storage-backend.html#backend-advanced-configuration-and-examples["FSx for NetApp ONTAP 백엔드 구성"].
...
다른 구성 옵션에 대한 자세한 내용은 아래 <<first-link,백엔드 고급 구성 및 예제>> 섹션을 참조하십시오.
...
== SMB 볼륨 제공
 ...
 `ontap-nas` 드라이버를 사용하여 SMB 볼륨을 프로비저닝할 수 있습니다. 하지만 그러려면 다음 단계를 완료해야 합니다: link:worker-node-prep.html#prepare-to-provision-smb-volumes["SMB 볼륨 프로비저닝을 위한 준비"].



[[first-linkbackend-advanced-configuration-and-examples]]
== [[first-link]]백엔드 고급 구성 및 예제
