...
summary: Google Cloud NetApp Volumes에서 스토리지를 프로비저닝하도록 Trident를 구성합니다. 
---
...
블록(iSCSI) 스토리지를 구성하려면 xref:trident-use/gcnv-block.html[블록 스토리지(iSCSI) 구성]을(를) 참조하십시오.
...
NOTE: Google Cloud NetApp Volumes UNIFIED 스토리지 풀의 경우, Trident는 볼륨 작업 중에 UNIFIED 전용 명명 및 검증 규칙을 적용합니다.
