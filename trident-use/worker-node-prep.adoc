...
.NFS 툴
link:https://docs.netapp.com/us-en/trident/trident-use/worker-node-prep.html#nfs-volumes["NFS 툴을 설치합니다"]다음을 사용하는 경우: `ontap-nas` , `ontap-nas-economy` , `ontap-nas-flexgroup` , 또는 `azure-netapp-files` .
...
.SMB 도구
link:https://docs.netapp.com/us-en/trident/trident-use/worker-node-prep.html#prepare-to-provision-smb-volumes["SMB 볼륨 프로비저닝을 위한 준비"] 다음을 사용하는 경우: `ontap-nas` SMB 볼륨을 프로비저닝합니다.
