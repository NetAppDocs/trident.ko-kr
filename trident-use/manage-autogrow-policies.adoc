...
[role="lead"]
자동 증가 정책을 생성한 후에는 해당 정책을 보고, 업데이트하고, 삭제할 수 있습니다. 또한 특정 정책을 사용하는 볼륨을 모니터링할 수도 있습니다.
...
IMPORTANT: 변경 사항은 해당 정책을 사용하는 모든 볼륨에 영향을 미칩니다. 가능한 경우, 먼저 비운영 환경에서 변경 사항을 테스트하십시오.
...
* *즉시 효력 발생:* 해당 정책을 사용하는 모든 볼륨은 다음 증가 평가 시 새 매개변수를 적용받습니다.
* *볼륨 재시작이 필요하지 않습니다.* 변경 사항은 다음 볼륨 확장 작업에 적용됩니다.
...
자동 확장 정책은 볼륨이 활발하게 사용되는 동안 실수로 삭제되는 것을 방지하기 위해 파이널라이저 보호 기능을 사용합니다.
...
----
. 볼륨이 여전히 해당 정책을 사용하는 경우 삭제 작업이 `Deleting` 상태가 됩니다. 어떤 볼륨이 영향을 받는지 확인하십시오.
+
[source, console]
----
tridentctl get autogrowpolicy production-db-policy -o yaml
...
| 볼륨은 정책을 사용합니다 | 정책이  `Deleting` 상태에 진입합니다. 최종화 도구는 모든 볼륨이 제거될 때까지 완료를 차단합니다. 
...
Autogrow는 ONTAP 백엔드를 지원합니다. 또한 Autogrow는 NAS 및 SAN 볼륨 모두에 대해 Google Cloud NetApp Volumes(GCNV)를 지원합니다.
...
* *기존 볼륨(브라운필드 배포):* 유효한 자동 증가 정책이 적용되었더라도 기존 볼륨에서는 자동 증가 기능이 작동하지 않을 수 있습니다. 이러한 제한은 볼륨 게시 마이그레이션이 진행 중이기 때문입니다. 마이그레이션이 완료되었는지 확인하려면 Trident 컨트롤러 로그에서 `"Migration completed"` 메시지를 확인하십시오.
...
PVC 어노테이션이 항상 우선합니다. PVC에  `trident.netapp.io/autogrowPolicy` 어노테이션이 있는 경우 Trident는 StorageClass에서 지정한 값과 관계없이 해당 값을 사용합니다. 자세한 내용은 link:automatic-volume-expansion.html#policy-precedence["정책 우선 순위"]를 참조하십시오.
...
PVC 어노테이션을  `"none"`로 설정하십시오. 이 설정은 해당 볼륨에 대한 StorageClass 수준 정책을 재정의합니다.
...
Trident 볼륨 확장을 중지합니다. 사용량이 계속 증가하더라도 해당 볼륨에 대한 추가 크기 조정 요청을 생성하지 않습니다 `usedThreshold`.

예를 들어, 이 정책을 사용하면 Trident는 볼륨이 100GiB에 도달한 후 더 이상 볼륨을 늘리지 않습니다.
...
예. 정책을 업데이트하면 해당 정책을 사용하는 모든 볼륨이 다음 용량 증가 평가 시 새 매개변수를 적용합니다. 볼륨을 재시작할 필요는 없습니다.
...
[[why-cant-i-delete-a-policy]]
=== 정책을 삭제할 수 없는 이유는 무엇인가요?

정책은 파이널라이저 보호 기능을 사용합니다. 볼륨이 여전히 정책을 사용하는 경우, 삭제 작업은  `Deleting` 상태에 들어가 정책에서 모든 볼륨을 제거할 때까지 기다립니다.
...
kubectl annotate pvc <pvc-name> \
  trident.netapp.io/autogrowPolicy-
----
모든 볼륨을 제거하면 Trident가 파이널라이저를 해제하고 정책을 삭제합니다.



[[which-backends-does-autogrow-support]]
=== autogrow는 어떤 백엔드를 지원합니까?

Autogrow는 ONTAP 백엔드를 지원합니다. 또한 NAS 및 SAN 볼륨 모두에 대해 Google Cloud NetApp Volumes(GCNV)도 지원합니다.

자동 확장 기능은 NFS, iSCSI, FCP 및 NVMe 프로토콜을 지원합니다. NVMe 원시 블록 볼륨을 사용하려면 ONTAP 9.16.1 이상 버전이 필요합니다.

기존 배포 환경의 기존 볼륨은 자동 확장이 적용되기 전에 볼륨 게시 마이그레이션을 완료해야 할 수 있습니다. 마이그레이션 상태를 확인하려면 Trident 컨트롤러 로그를 확인하십시오.
