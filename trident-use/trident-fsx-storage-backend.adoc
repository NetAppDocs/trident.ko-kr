...
Trident를 사용하여 볼륨을 프로비저닝하려면 먼저 Trident 백엔드 구성(TBC)을 생성하여 스토리지 백엔드를 Trident와 통합해야 합니다. TBC는 Trident가 스토리지 백엔드에 연결하는 방법을 정의하고 Trident가 스토리지 리소스를 관리하는 데 필요한 정보를 제공합니다.



[[create-a-trident-backend-configuration-tbc-for-fsx-for-ontap]]
== FSx for ONTAP용 Trident 백엔드 구성(TBC) 생성

Trident 백엔드가 FSx for ONTAP 파일 시스템을 사용하도록 구성하는 방법은 두 가지입니다. * 자동 백엔드 기능을 활용하는 StorageClass를 생성합니다. * 백엔드와 StorageClass를 별도로 생성합니다.

이 섹션의 단계에서는 백엔드를 별도로 생성하는 방법을 다룹니다. 자동 백엔드 기능 사용 방법에 대한 자세한 내용은 link:trident-fsx-storageclass-pvc.html#configure-a-storage-class-without-automatic-backend-configuration["StorageClass 및 PVC 구성"] 섹션을 참조하십시오.
...
| 'torageDriverName'입니다 | 스토리지 드라이버의 이름입니다. 생성 후에는 수정할 수 없습니다. 이 매개변수를 업데이트하려면 새 백엔드를 생성해야 합니다. | `ontap-nas`, `ontap-nas-economy`, `ontap-nas-flexgroup`, `ontap-san`, `ontap-san-economy` 
...
| 다타LIF | 프로토콜 LIF의 IP 주소입니다. * ONTAP NAS 드라이버*: NetApp dataLIF를 지정하는 것을 권장합니다. 지정하지 않으면 Trident는 SVM에서 dataLIF를 가져옵니다. NFS 마운트 작업에 사용할 정규화된 도메인 이름(FQDN)을 지정하여 여러 dataLIF에 걸쳐 로드 밸런싱을 수행하는 라운드 로빈 DNS를 구성할 수 있습니다. 초기 설정 후 변경할 수 있습니다. 을 참조하십시오. https://kb.netapp.com/Cloud/Astra/Trident/Is_it_possible_to_update_the_DataLIF_on_the_trident_backend["초기 구성 후 dataLIF 업데이트"^] * ONTAP SAN 드라이버*: iSCSI의 경우 지정하지 마십시오. Trident는 ONTAP 선택적 LUN 맵을 사용하여 멀티 패스 세션을 설정하는 데 필요한 iSCSI LIF를 검색합니다. dataLIF가 명시적으로 정의된 경우 경고가 생성됩니다. Trident를 IPv6 플래그를 사용하여 설치한 경우 IPv6 주소를 사용하도록 설정할 수 있습니다. IPv6 주소는 [28e8:d9fb:a825:b7bf:69a8:d02f:9e7b:3555]와 같이 대괄호 안에 정의해야 합니다. |  
...
| '토르agePrefix' | SVM에서 새 볼륨을 프로비저닝할 때 사용되는 접두사 생성 후에는 수정할 수 없습니다. 이 매개 변수를 업데이트하려면 새 백엔드를 생성해야 합니다. | `trident`참고: 백엔드 ID 및 볼륨 이름 지정에 영향을 미치는 백엔드 구성 매개변수(예: `storageDriverName` 및 `storagePrefix`)는 생성 후 변경할 수 없습니다. 이러한 매개변수를 업데이트하려면 새 백엔드를 생성해야 합니다. 
...
| `luksEncryption` | LUKS 암호화를 활성화합니다. 다음을 참조하십시오. link:../trident-reco/security-luks.html["LUKS(Linux Unified Key Setup) 사용"]. SAN 전용입니다. | "" 
...
== SMB 볼륨 제공
 ...
 `ontap-nas` 드라이버를 사용하여 SMB 볼륨을 프로비저닝할 수 있습니다. link:trident-fsx-storage-backend.html#fsx-for-ontap-driver-details["ONTAP SAN 및 NAS 드라이버 통합"]를 완료하기 전에 다음 단계를 완료하십시오: link:worker-node-prep.html#prepare-to-provision-smb-volumes["SMB 볼륨 프로비저닝을 위한 준비"].
