...
이 섹션의 단계에서는 백엔드를 별도로 생성하는 방법을 다룹니다. 자동 백엔드 기능 사용 방법에 대한 자세한 내용은 xref:trident-fsx-storageclass-pvc.html#configure-a-kubernetes-storageclass-object-with-automatic-backend-configuration-support-for-fsx-for-ontap[StorageClass 및 PVC 구성] 섹션을 참조하십시오.
...
다음을 사용하여 SMB 볼륨을 프로비저닝할 수 있습니다. `ontap-nas` 운전사. 완료하기 전에 <<ONTAP SAN and NAS driver integration>> 다음 단계를 완료하세요. link:https://docs.netapp.com/us-en/trident/trident-use/worker-node-prep.html#prepare-to-provision-smb-volumes["SMB 볼륨 프로비저닝을 위한 준비"].
