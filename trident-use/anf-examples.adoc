...
| `maxThroughput` | 허용되는 최대 처리량을 설정합니다. 수동 QoS 용량 풀에서만 지원됩니다. * Trident 25.06 기술 미리보기* | `4 MiB/sec` 
|===
...
[%collapsible]
====
이 백엔드 구성은 Azure의 볼륨을 배치합니다.  `eastus` 수동 QoS 용량 풀을 사용한 위치. * NetApp Trident 25.06의 기술 미리보기*.
