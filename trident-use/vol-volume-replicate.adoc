...
. 기본 Kubernetes 클러스터에서 TridentTridentSnapshotInfoCR을 생성하여 내부 정보를 얻으십시오.
+
.예
[source, yaml]
----
...
----
+
.. 보조 Kubernetes 클러스터에서 _TridentMirrorRelationship_ CR의 _spec.state_ 필드를 _promoted_로 업데이트하고 _spec.volumeMappings[].promotedSnapshotHandle_을 스냅샷의 internalName으로 설정합니다.
