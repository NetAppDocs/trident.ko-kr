...
일반적으로 초기 설치에 사용했던 것과 동일한 방법으로 업그레이드하는 것이 좋지만, link:../trident-install/kubernetes-deploy.html#moving-between-installation-methods["설치 방법 간에 이동합니다"]. Trident를 업그레이드하는 두 가지 옵션이 있습니다.
